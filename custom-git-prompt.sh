...
__posh_git_echo () {
    ...
    local arrow_color="$fg[yellow]"

    # branch
    if (( $__POSH_BRANCH_BEHIND_BY > 0 && $__POSH_BRANCH_AHEAD_BY > 0 )); then
        gitstring+="$fg[cyan]$bg[yellow]\ue0b0 $fg[black]\ue0a0 "
        gitstring+="$branchstring"
        gitstring+=" \u2193$__POSH_BRANCH_BEHIND_BY \u2191$__POSH_BRANCH_AHEAD_BY"
    elif (( $__POSH_BRANCH_BEHIND_BY > 0 )); then
        arrow_color="$fg[red]"
        gitstring+="$fg[cyan]$bg[red]\ue0b0 $fg[black]\ue0a0 "
        gitstring+="$branchstring"
        gitstring+=" \u2193$__POSH_BRANCH_BEHIND_BY"        
    elif (( $__POSH_BRANCH_AHEAD_BY > 0 )); then
        arrow_color="$fg[green]"
        gitstring+="$fg[cyan]$bg[green]\ue0b0 $fg[black]\ue0a0 "
        gitstring+="$branchstring"
        gitstring+=" \u2191$__POSH_BRANCH_AHEAD_BY"        
